...
class GenericDataset(data.Dataset):
    def __init__(
        self, dataset_name, split, random_sized_crop=False, num_imgs_per_cat=None
    ):
    ...
    def _keep_first_k_examples_per_category(self, num_imgs_per_cat):
        ...
        if self.dataset_name == "cifar10":
            ...
            raise ValueError(
                "Keeping k examples per category has not been implemented for the {0}".format(
                    dname
                )
            )
...
class DataLoader(object):
    def __init__(
        ...
        self.transform = transforms.Compose(
            [transforms.ToTensor(), transforms.Normalize(mean=mean_pix, std=std_pix)]
        )
    ...
    def get_iterator(self, epoch=0):
        ...
        tnt_dataset = tnt.dataset.ListDataset(
            elem_list=range(self.epoch_size), load=_load_function
        )
