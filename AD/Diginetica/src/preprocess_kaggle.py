...
if __name__ == "__main__":
    ...
    event_df["valid_session"] = event_df.session_id.map(
        event_df.groupby("session_id")["item_id"].size() > 2
    )
    event_df = event_df.loc[event_df.valid_session].drop("valid_session", axis=1)
    ##########remove items with <=2 interactions
    event_df["valid_item"] = event_df.item_id.map(
        event_df.groupby("item_id")["session_id"].size() > 2
    )
